...
def jacobi(u, interior_mask, max_iter, atol=1e-6):
    ...
    for i in range(max_iter):
        ...
        delta = cp.abs(u[1:-1, 1:-1][interior_mask] - u_new_interior).max() # delta will now be on gpu
...
def summary_stats(u, interior_mask):
    ...
    return {
        'mean_temp': float(mean_temp.get()),
        'std_temp': float(std_temp.get()),
        'pct_above_18': float(pct_above_18.get()),
        'pct_below_15': float(pct_below_15.get()),
    }
...
if __name__ == '__main__':
    ...
    #for i in range(3):
        # Plot interior mask
    #    ax = axes[i, 0]
    #    im1 = ax.imshow(all_interior_mask[i].get(), cmap='gray')
    #    ax.set_title(f"all_interior_mask[{i}]")
    #    ax.axis("off")
    #    # Plot u0 (full grid with boundary)
    #    ax = axes[i, 1]
    #    im0 = ax.imshow(all_u0[i].get(), cmap='viridis')
    #    ax.set_title(f"all_u0[{i}]")
    #    ax.axis("off")
    #    fig.colorbar(im0, ax=ax)
    #    # Plot u (full solution grid)
    #    ax = axes[i, 2]
    #    im1 = ax.imshow(all_u[i].get(), cmap='viridis')
    ...
    # Save plot
    #output_path = 'visualizaion_cupy.png'
